...
corporate-policy documents and 30 ground-truth question--answer pairs. We
...
\begin{itemize}
  ...
  \item An empirical finding that contradicts the "reranking always helps"
        ...
        sparse channel adds almost nothing, so retrieval-stage value is
        corpus-dependent.
\end{itemize}
...
The practical consequence is that the sparse channel contributes little to no
useful signal in our runs; we establish this empirically in
Section~\ref{sec:results} and discuss it in Section~\ref{sec:discussion}.
...
handbook, and an employee onboarding guide. The documents are clean,
...
\emph{Coverage.} Two hybrid cells are each computed over 29 of 30 questions, for
two unrelated reasons. The hybrid faithfulness mean omits Q13 (an IT-helpdesk
question), whose faithfulness scoring timed out and is recorded as missing, and
the hybrid context-precision mean omits Q2 (an HR question), which has no recorded
context-precision score. Both dense and rerank are computed over the full 30 on
each metric. We report these gaps explicitly rather than imputing values.
...
B: Hybrid          & \textbf{0.886}\textsuperscript{a} & 0.928\textsuperscript{b} & \textbf{0.907} \\
C: Hybrid$+$Rerank & 0.750          & 0.900          & 0.825 \\
\bottomrule
\end{tabular}
\\[2pt]
\begin{minipage}{0.95\linewidth}
\footnotesize \textsuperscript{a}Hybrid faithfulness is a mean over 29 questions;
Q13 timed out during faithfulness scoring (recorded as missing).
\textsuperscript{b}Hybrid context precision is a mean over 29 questions; Q2 has no
recorded context-precision score. All dense and rerank cells are over 30.
\end{minipage}
\end{table}

Three observations follow directly from the data.

\textbf{(1) Hybrid attains the best average, but by a razor-thin margin.}
Hybrid's average (0.907) edges out dense (0.905) by 0.002, a margin far smaller
than plausible run-to-run variation. The two per-metric values appear to point in
opposite directions: hybrid is nominally higher on faithfulness (0.886 vs.\ 0.873)
and nominally \emph{lower} on context precision (0.928 vs.\ 0.936). As finding~(2)
shows, however, the context-precision gap is an artifact of differential coverage
rather than a retrieval difference, so the apparent disagreement between the two
metrics is not real.
...
faithfulness score is missing. Adding the sparse retriever and RRF fusion thus
left the context fed to the generator unchanged on every question that was
actually scored. Context precision makes this sharper: that judge is deterministic
given the contexts, so on the 28 byte-identical questions for which both methods
carry a context-precision score it returns \emph{identical} values for A and B
(28 of 28). The small context-precision gap in Table~\ref{tab:results}
(0.936 vs.\ 0.928) is therefore not a retrieval difference and not judging noise,
but a coverage artifact driven by the two questions outside that identical set:
Q2, which has no hybrid context-precision score, and Q13, whose contexts differ.
Faithfulness is answer-dependent rather than fixed by the context set, so the
small A/B faithfulness difference can still reflect generation stochasticity; the
context-precision side cannot. Either way, A and B are even more equivalent than a
raw table read suggests, which is the empirical footprint of the sparse-channel
index mismatch noted in Section~\ref{sec:methods}.
...
routinely collapse to one or two distinct parent blocks. Empirically, this is a
mean of 1.73 contexts for Method~C versus about 8 for Methods~A and~B. With only one or
...
We are equally clear that this margin is thin (0.002), and that our context-level
analysis shows dense and hybrid retrieving identical contexts on every scored
question and earning identical context-precision scores wherever those contexts
coincide. The nominal split in Table~\ref{tab:results}, dense ahead on context
precision and hybrid ahead on faithfulness, is not a genuine disagreement: the
context-precision gap is a coverage artifact (Section~\ref{sec:results}), and only
the faithfulness side reflects an answer-level difference. The honest reading is
that, on this
